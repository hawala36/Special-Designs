...
let backgroundButtons = document.querySelectorAll(
  "nav .random-background .buttons button"
);
...
if (Object.keys(window.localStorage).includes("background")) {
  let backgroundSelected = document.querySelector(
    `nav .random-background .buttons .${window.localStorage.getItem(
      "background"
    )}`
  );
  backgroundSelected.classList.add("selected");
} else {
  let yesBackground = document.querySelector(
    `nav .random-background .buttons .yes`
  );
  yesBackground.classList.add("selected");
}

let bulletsButtons = document.querySelectorAll(
  "nav .show-bullets .buttons button"
);
...
if (Object.keys(window.localStorage).includes("bullets")) {
  let bulletSelected = document.querySelector(
    `nav .show-bullets .buttons .${window.localStorage.getItem("bullets")}`
  );
...
resetNav.onclick = function () {
  ...
  document.documentElement.style.setProperty(
    "--mainColor",
    colors[0].getAttribute("data-set")
  );
...
if (Object.keys(window.localStorage).includes("color") === false) {
  ...
  let localSelectedColor = document.querySelector(
    `nav .colors-container [data-color = "${window.localStorage.getItem(
      "color"
    )}"]`
  );
...
for (let i = 0; i < photoDivs.length; i++) {
  photoDivs[i].onclick = function () {
    ...
    scaledImage.setAttribute(
      "src",
      photoDivs[i].children.item(0).getAttribute("src")
    );
